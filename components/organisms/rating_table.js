...
export default function Rating_table(props) {
    ...
    const data = props.data.sort((a, b) => b.all - a.all);
    ...
    const reviewAddColor =(level)=>{
        switch(level){
            ...
            case "Positive":
                return{color:"#4FA552",fontSize:"16px"}
            case "Mostly Positive":
                return{color:"#84A54F",fontSize:"16px"}
            case "Mixed":
                return{color:"#C4C167",fontSize:"16px"}
            case "Mostly Negative":
                return{color:"#FDAE58",fontSize:"16px"}
            case "Negative":
                return{color:"#F75435",fontSize:"16px"}
            case "Very Negative":
                return{color:"#AC0202",fontSize:"16px"}
            case "Overwhelmingly Negative":
                return{color:"#780202",fontSize:"16px"}
            default:return{}
        }
    }
                            ...
                            <div style={{paddingTop:"12px", paddingRight:"4%", paddingLeft:"4%"}} >
                                <div style={{ backgroundColor:"white", borderRadius:"10px", width: "100%", display:"flex", minHeight:"80px",paddingLeft:"3%", paddingRight:"3%", height:"auto"}} >
                                    <div style={{ width:"60%", margin:"auto"}}>
                                        <h5 style={{fontSize:"16px", color:"black"}}>{data.name}</h5>
                                    </div>

                                    <div style={{ width:"40%", margin:"auto", textAlign:"center"}}>
                                        <h5 style={reviewAddColor(data.review)}>{data.review}</h5>
                                    </div>
                                    <div style={{ width:"30%", margin:"auto", textAlign:"center"}}>
                                        <h5 style={{fontSize:"16px", color:"black"}}>{data.all.toLocaleString()}</h5>
                                    </div>
                                </div>
                            </div>
                        </div>
